      ...
      <div class="col-sm-12">
         <div class="card">
            <div class="card-header">
               ...
               <span>With DataTables you can alter the ordering characteristics of the table at initialisation time. Using the <code class="option" title="DataTables initialisation option">order:option</code> initialisation parameter, you can set
               the table to display the data in exactly the order that you want.</span><span>The<code class="option" title="DataTables initialisation option">order:Option</code> parameter is an array
               of arrays where the first value of the inner array is the column to order on, and the second is <code class="string" title="String">'asc':String</code> (ascending
               ordering) or <code class="string" title="String">'desc':String</code> (descending ordering) as required.<code class="option" title="DataTables initialisation option">order:String</code> is a 2D array to allow multi-column
               ordering to be defined.</span>
            </div>
            <div class="card-body">
               <div class="table-responsive">
      ...
      <div class="col-sm-12">
         <div class="card">
            <div class="card-header">
               ...
               <span>DataTables allows ordering by multiple columns at the same time, which can be activated in a number of different ways:</span><span>User shift click on a column (added the clicked column as a secondary, tertiary etc ordering column).</span><span>On a per-column basis (i.e. order by a specific column and then a secondary column if the data in the first column is identical), through the<code>columns.orderData option.</code></span><span>Using the <code>columns.orderData option</code>option to specify a multiple column order by default (for example [ 0, 1 ]).</span><span>Through the <code>order() API </code>API method.</span><span>Note that, the ability for the user to shift click to order multiple columns can be disabled through the <code>orderMulti option</code>option.</span><span>The example below shows the first column having a secondary order applied to the second column in the table, vice-versa for the second column being tied directly to the first and the salary column to the first name column.</span>
            </div>
            <div class="card-body">
               <div class="table-responsive">
      ...
      <div class="col-sm-12">
         <div class="card">
            <div class="card-header">
               ...
               <span>There are times when you might find it useful to display only a sub-set of the information that was available in the original table. For example you might want to reduce the amount of data shown on screen to make it clearer for the user (consider also using the Responsive extension for this). This is done through the<code> columns.visible column option</code>option.</span><span>The column that is hidden is still part of the table and can be made visible through the api column().visible() API method at a future time if you wish to have columns which can be shown and hidden.</span><span>Furthermore, as the hidden data is still part of the table, it can still, optionally, be filtered upon allowing the user access to that data (for example 'tag' information for a row entry might used).</span><span>In the table below both the office and age version columns have been hidden, the former is not searchable, the latter is.</span>
            </div>
            <div class="card-body">
               <div class="table-responsive">
      ...
      <div class="col-sm-12">
         <div class="card">
            <div class="card-header">
               ...
               <span>When using tables to display data, you will often wish to display column information in groups. DataTables fully supports <code>colspan</code> and<code>rowspan</code> in the table's header, assigning the required order listeners to the TH element suitable for that column.</span><span>
      ...
      <div class="col-sm-12">
         <div class="card">
            <div class="card-header">
               ...
               <span>
               When customising DataTables for your own usage, you might find that the default position of the feature elements (filter input etc) is not quite to your
               liking. To address this issue DataTables takes inspiration from the CSS 3 Advanced Layout Module and provides the <code class="option" title="DataTables initialisation option">dom:option</code> initialisation parameter which can be set
               to indicate where you wish particular features to appear in the DOM. You can also specify <code>div</code> wrapping containers (with an id and / or class) to
               provide complete layout flexibility.</span><span>Each HTML control element presented by DataTables is denoted by a single character in the <code class="option" title="DataTables initialisation option">dom:option</code> option. For example the <code>l</code> option is used for the <code>L</code>ength changing
               input option.</span>
            </div>
            <div class="card-body">
               <div class="table-responsive">
      ...
      <div class="col-sm-12">
         <div class="card">
            <div class="card-header">
               ...
               <span>Often you may want to have your table resize dynamically with the page. Typically this is done by assigning <code>width:100%</code> in your CSS, but this
               presents a problem for Javascript since it can be very hard to get that relative size rather than the absolute pixels. As such, if you apply the<code>width</code> attribute to the HTML table tag or inline width style (<code>style="width:100%"</code>), it will be used as the width for the table
               (overruling any CSS styles).</span><span>This example shows a table with <code>width="80%"</code> and the container is also flexible width, so as the window is resized, the table will also resize
               dynamically.</span>
            </div>
            <div class="card-body">
               <div class="table-responsive">
